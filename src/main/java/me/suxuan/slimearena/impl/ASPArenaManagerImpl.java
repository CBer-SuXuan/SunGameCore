	...
	public @NotNull CompletableFuture<Void> discardArenaAsync(@NotNull World world, @Nullable Location fallbackLocation) {
		...
		Bukkit.getScheduler().runTask(plugin, () -> {
			...
			Bukkit.getScheduler().runTaskLater(plugin, () -> {
				// ==========================================
				// 【核心插入】处理 WorldGuard 强行占用内存的问题
				// ==========================================
				if (Bukkit.getPluginManager().getPlugin("WorldGuard") != null) {
					try {
						// 强行卸载 WG 在该世界的 Region 缓存
						WorldGuard.getInstance().getPlatform().getRegionContainer().unload(
								BukkitAdapter.adapt(world)
						);
						plugin.log("<gray>已清理 WorldGuard 在 <aqua>" + worldName + "</aqua> 的区划缓存。</gray>");
